...
class GameOrchestrator:
    ...
    async def _process_game_turn(self, player_input: str):
        ...
        print(f"[Profile] Tổng thời gian Turn: {time.perf_counter() - start_turn_time:.3f}s")
    ...
    async def run(self):
        ...
        print("[Hệ thống] Đang dọn dẹp dữ liệu cũ...")
        self.db.reset_database()
        self.image_manager.clear_image_folders()
        self.db.create_tables()  # Đảm bảo bảng được tạo lại nếu vừa bị xóa
        ...
        self.db.add_location_to_db(starting_loc_obj)
        ...
        while True:
            ...
            if player_input.lower() in ['exit', 'quit', 'thoát']:
                ...
                self.db.conn.close()  # Đóng kết nối để không bị lock database (file WAL)
                break
